...
const DialogPortal = ({
  ...
  <DialogPrimitive.Portal className={cn(className)} {...props}>
    <div className="fixed inset-0 z-50 flex justify-center sm:items-center items-start mt-10">
      {children}
    </div>
...
>(({ className, closeBtn = true, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'animate-in data-[state=open]:fade-in-90 zoom-in-90 data-[state=open]:slide-in-from-bottom-0 fixed z-50 grid w-full gap-4 rounded-b-lg bg-white p-6 max-w-lg rounded-lg',
        'dark:bg-slate-900',
        className
      )}
      {...props}
